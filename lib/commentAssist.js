...
function fixChildNames(text, students, preferredStudentId) {
  ...
  return text.replace(/\b[A-Za-z][A-Za-z'-]*\b/g, (raw) => {
    ...
    for (const stu of list) {
      const parts = stu.full.split(/\s+/).filter((p) => p.length >= 2);
      for (const part of parts) {
        const pl = part.toLowerCase();
        const d = levenshtein(wLower, pl);
        const thresh = maxEditsForLen(Math.max(raw.length, part.length));
        if (d <= thresh && d > 0) {
          candidates.push({ stu, full: stu.full, d, part });
        }
      }
    }
    if (!candidates.length) return raw;
    candidates.sort((a, b) => a.d - b.d || a.full.length - b.full.length);
    const bestD = candidates[0].d;
    const top = candidates.filter((c) => c.d === bestD);
    if (top.length > 1) {
      const pref = top.filter((c) => c.stu.id === preferredStudentId);
      if (pref.length === 1) {
        return preserveCasePattern(raw, pref[0].full);
      }
      const ids = new Set(top.map((c) => c.stu.id));
      if (ids.size > 1) return raw;
    }
    const pick = top[0];
    return preserveCasePattern(raw, pick.full);
  });
}

/** If original was ALL CAPS, uppercase result; else use canonical full_name casing */
function preserveCasePattern(original, fullName) {
